...
import { PieChart, Shield, Calculator } from "lucide-react";
import RetirementGapChecker from "@/components/sections/RetirementGapChecker";

const ICONS = { PieChart, Shield, Calculator } as const;

export default function RetirementPlanningPage() {
  const data = servicePages.find((p) => p.id === "retirement-planning")!;
  return (
    <div>
      ...
      <Section title="Key benefits">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.benefits.map((b) => {
            ...
            const ICONS: Record<string, IconCmp> = { PieChart, Shield, Calculator };
            const Icon = ICONS[b.icon] ?? PieChart;
            return (
      ...
      <Section title="Savings Gap Checker">
