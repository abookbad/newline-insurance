...
const ICONS = { ScrollText, Users, ShieldCheck } as const;

export default function EstateLegacyPlanningPage() {
  const data = servicePages.find((p) => p.id === "estate-legacy-planning")!;
  return (
    <div>
      ...
      <Section title="Overview">
        <div className="grid gap-4 max-w-3xl">
          {data.overview.map((p) => (<p key={p} className="text-black/80">{p}</p>))}
        </div>
      </Section>
      <Section title="Key benefits">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.benefits.map((b) => {
            ...
            const ICONS: Record<string, IconCmp> = { ScrollText, Users, ShieldCheck };
            const Icon = ICONS[b.icon] ?? ScrollText;
            return (
      ...
      <Section title="Beneficiary & Document Checklist">
