...
import { Calculator, ScrollText, Shield } from "lucide-react";

export default function RealEstatePlanningPage() {
  const data = servicePages.find((p) => p.id === "real-estate-planning")!;
  return (
    <div>
      ...
      <Section title="Key benefits">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.benefits.map((b) => {
            const ICONS: Record<string, React.ComponentType<React.SVGProps<SVGSVGElement>>> = { Calculator, ScrollText, Shield };
            const Icon = ICONS[b.icon] ?? Calculator;
            return (
