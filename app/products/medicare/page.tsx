...
import { BadgeInfo, HeartPulse, ShieldCheck } from "lucide-react";
import MedicareWindowChecker from "@/components/sections/MedicareWindowChecker";

const ICONS = { BadgeInfo, HeartPulse, ShieldCheck } as const;

export default function MedicarePage() {
  const data = servicePages.find((p) => p.id === "medicare")!;
  return (
    <div>
      ...
      <Section title="Key benefits">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.benefits.map((b) => {
            ...
            const ICONS: Record<string, IconCmp> = { BadgeInfo, HeartPulse, ShieldCheck };
            const Icon = ICONS[b.icon] ?? BadgeInfo;
            return (
      ...
      <Section title="Enrollment Window Checker">
